...
- Adding method effects to a stream information

  You can [add sound effects](#stream_effect_method) to the stream information
...
## Adding effect methods to a stream information
...
There are two types of APIs that you can use.

- In case of exclusive methods named `sound_effect_method_with_reference_e`, It includes Acoustic Echo Cancellation(AEC) functionality and it can not be set with other methods because it uses a playback reference that is a exclusive system resource.
- In case of `sound_effect_method_e`, There are like Noise-Suppression(NS) and Automatic Gain Control(AGC) that don't need a reference playback. Therefore, you can use these methods simultaneously using 'or' keyword.

1. Add effect method(AEC) with a reference device to a stream information
...
2. Add effect method(NS and AGC) simultaneously to a stream information
